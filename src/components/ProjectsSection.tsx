...
const ProjectCard = ({ project }: { project: typeof projects[0] }) => {
  return (
    <div className="group relative glass-strong rounded-xl overflow-hidden border border-border hover:border-primary/50 transition-all duration-300 hover:-translate-y-1">
      {/* Image */}
      <div className="relative h-48 overflow-hidden bg-secondary/50">
        ...
        {/* Overlay buttons */}
        <div className="absolute inset-0 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300 bg-background/80 backdrop-blur-sm">
          <a
            href={project.githubUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="p-3 rounded-lg bg-primary text-primary-foreground hover:scale-110 transition-transform"
          ...
          {project.liveUrl !== "#" && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-3 rounded-lg bg-secondary text-foreground hover:scale-110 transition-transform border border-border"
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          {project.featured && (
            <span className="px-2.5 py-1 text-xs font-semibold rounded-lg bg-primary text-primary-foreground">
              Featured
            </span>
          )}
          {project.badge && (
            <span className="px-2.5 py-1 text-xs font-semibold rounded-lg bg-accent text-accent-foreground">
              {project.badge}
            </span>
          )}
        </div>
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {project.techStack.map((tech) => (
            <span
              key={tech}
              className="px-2.5 py-1 text-xs font-medium rounded-md bg-secondary/80 text-muted-foreground border border-border"
            >
              {tech}
            </span>
          ))}
